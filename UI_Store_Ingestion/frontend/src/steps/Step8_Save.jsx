...
export default function Step8_Save() {
  ...
  const { setStep, loadProject: hydrateProject, floorPlanWidth, floorPlanHeight, pixelsPerMeter, origin, zones, obstacles, cameras } = store
  ...
  const handleLoad = async () => {
    try {
      ...
      else alert('No project file found on server.')
    } catch (e) {
      alert(e.message)
    }
  }
  ...
  return (
    <div className="max-w-3xl">
      ...
      <p className="text-gray-500 mb-6 text-sm">Review and save your project configuration.</p>
      ...
      <div className="space-y-4 mb-8">
        ...
        <Section title={`Zones (${zones.length})`}>
          {zones.length === 0 ? <p className="text-sm text-gray-400 py-1">No zones defined.</p> :
            zones.map(z => <Row key={z.id} label={z.name} value={`${z.type} · ${z.points.length} vertices`} />)}
        </Section>

        <Section title={`Obstacles (${obstacles.length})`}>
          {obstacles.length === 0 ? <p className="text-sm text-gray-400 py-1">No obstacles defined.</p> :
          ...
          {cameras.map(c => (
            <div key={c.id} className="py-1 text-sm border-b last:border-0">
              ...
              <span className="text-gray-400 ml-2">
                ...
                <span className={c.homographyStatus === 'ok' ? 'text-green-600' : 'text-red-500'}> {c.homographyStatus || 'no homography'}</span>
              </span>
            </div>
          ))}
        </Section>
      ...
      <div className="flex gap-3 flex-wrap">
        <button onClick={handleSave} disabled={loading} className="px-5 py-2.5 bg-blue-600 text-white rounded-lg font-medium disabled:opacity-40 hover:bg-blue-700 transition">
          {loading ? 'Saving...' : '💾 Save Project'}
        </button>
        <button onClick={handleExport} className="px-5 py-2.5 bg-gray-100 text-gray-700 rounded-lg font-medium hover:bg-gray-200 transition">
          📥 Export JSON
        </button>
        <button onClick={handleLoad} className="px-5 py-2.5 bg-gray-100 text-gray-700 rounded-lg font-medium hover:bg-gray-200 transition">
          📂 Load Existing
        </button>
      </div>

      {saved && <p className="mt-3 text-green-600 text-sm font-medium">✓ Project saved to project.json on the backend.</p>}
