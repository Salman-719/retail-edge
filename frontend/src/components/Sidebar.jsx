...
import {
  ...
  UserCheck, ClipboardList, SlidersHorizontal, ChevronLeft, Video, Bell,
} from 'lucide-react'
...
export default function Sidebar() {
  ...
  return (
    ...
    >
        ...
        {NAV_GROUPS.map((group, gi) => (
          <div key={gi}>
            ...
            <div className="flex flex-col gap-0.5">
              {group.items
                .filter(({ path }) => path !== 'settings' || isAdmin)
                .map(({ label, path, Icon }) => (
                <NavLink
                  key={path}
                  to={`/store/${slug}/${path}`}
      ...
      {/* Admin-only dev nav — gated by super-admin (A5). Backend routes are admin-gated (A4). */}
      {isAdmin && (
      ...
      <div className="px-2 py-3 border-t border-white/10 flex flex-col gap-0.5">
        {(state.user?.account_type === 'owner' || isAdmin) && (
          <button
