...
def _apply_iep4_k8s(store_id: str) -> None:
    # TODO(k8s deploy): create-or-patch an IEP4 StatefulSet (replicas=1,
    # serviceAccount 'iep4', env from _env(), SERVER_REDIS_URL not needed) —
    # mirror iep3_manager.apply_iep3. Stubbed until cluster deploy.
    log.warning(
        "iep4_manager: k8s StatefulSet path not implemented yet — TODO (store=%s)", store_id
    )
...
def delete_iep4(store_id: str) -> None:
    ...
    if _apps_v1 is not None:
        # TODO(k8s deploy): delete the IEP4 StatefulSet. Stubbed.
        log.warning("iep4_manager: k8s delete path not implemented yet — TODO (store=%s)", store_id)
        return
    if _docker_client is None:
        return
    import docker.errors  # type: ignore[import-untyped]
