...
def _run_iep5_k8s(store_id: str, shift_date: str) -> str | None:
    """Create a batch/v1 Job for (store_id, shift_date).

    backoffLimit=3 (k8s retries on failure), ttlSecondsAfterFinished=86400
    (auto-cleanup after a day), restartPolicy=Never. Idempotent: any prior Job
    with the same name is deleted first so a re-trigger re-runs cleanly.
    Env from _env() — values already resolved in the EEP process.
    """
    from kubernetes import client as k8s  # type: ignore[import-untyped]
    from kubernetes.client.exceptions import ApiException  # type: ignore[import-untyped]

    name   = _job_name(store_id, shift_date)
    labels = {"app": "iep5", "store-id": store_id, "shift-date": shift_date}
    env    = [k8s.V1EnvVar(name=k, value=v) for k, v in _env(store_id, shift_date).items()]

    # Idempotency: remove a prior Job (and its pods) with the same name.
    try:
        _batch_v1.delete_namespaced_job(
            name, _NAMESPACE,
            propagation_policy="Background",
        )
        log.info("iep5_manager: removed prior Job %s before re-run", name)
    ...
    try:
        _batch_v1.create_namespaced_job(_NAMESPACE, job)
        log.info("iep5_manager: launched Job %s (store=%s date=%s)", name, store_id, shift_date)
        return name
    except ApiException as exc:
        log.error("iep5_manager: failed to create Job %s: %s", name, exc)
        raise
