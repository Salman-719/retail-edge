...
### Reconnecting kubectl later (IP changed) {#reconnect-kubectl}

Your laptop's public IP changes often (new network, DHCP, VPN). When `kubectl`
suddenly **times out** on `:6443`, your IP fell out of the allow-list. Re-run
this any time to whitelist your current IP — safe to repeat:
...
export CLUSTER_SG=$(aws ec2 describe-instances --region $AWS_REGION \
  ...
  --query 'Reservations[].Instances[].SecurityGroups[].GroupId' --output text)
aws ec2 authorize-security-group-ingress --region $AWS_REGION \
  --group-id $CLUSTER_SG --protocol tcp --port 6443 \
  --cidr $(curl -s https://checkip.amazonaws.com)/32 2>&1 | grep -v Duplicate || true
kubectl get nodes   # should connect now
```
...
This creates the **per-store IEP3 reconciliation pod**, keyed to the store's UUID.

> **Order matters:** the store's UUID is generated by EEP when you create the
> store in the GUI (Step 10) — the "New Store" form has no UUID field. So create
> the store in the GUI first, then come back here with its real UUID. Deploying
> IEP3 against a made-up UUID won't match the store, and reconciliation won't run.

Get the store's UUID after creating it in the GUI:

```bash
# RDS_HOST / RDS_PASS come from Step 5 (re-run that block if your shell is fresh)
kubectl run psql --rm -it --restart=Never --image=postgres:15 \
  --env="PGPASSWORD=$RDS_PASS" -- \
  psql -h $RDS_HOST -U retailvision -d retailvision \
  -c "SELECT id, slug, name FROM stores ORDER BY created_at;"

export STORE_ID=<the-id-of-your-store>
```

Then deploy IEP3 for that store:
